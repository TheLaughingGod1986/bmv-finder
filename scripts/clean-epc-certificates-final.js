...
/**
 * Final Memory-Optimized EPC Certificates Cleaning Script
 * 
 * Uses streaming file-based sorting to handle 20M+ unique properties
...
// Streaming merge of sorted files
const streamingMerge = async (chunkFiles) => {
    log('Starting streaming merge of sorted chunks...');
    ...
    // Track seen properties for global deduplication
    const seenProperties = new Set();
    let written = 0;
    
    // Process each chunk file in order (since they're sorted)
    ...
    log(`Streaming merge complete. Final unique records: ${written.toLocaleString()}`);
    return written;
};

// Main cleaning function
const cleanEpcData = async () => {
    log('Starting final EPC data cleaning process...');
    ...
    // Stream merge all chunks
    await streamingMerge(chunkFiles);
