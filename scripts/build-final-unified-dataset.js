...
const ALIGNED_DATA_DIR = 'data/aligned-2006-final';
...
// Focus on a single postcode area with good coverage
const TARGET_POSTCODE_PREFIX = 'LE13'; // Leicestershire area

console.log('🚀 Building final unified dataset with address format handling...');
...
async function filterLandRegistryByPostcode() {
  console.log(`📖 Filtering Land Registry data by postcode prefix: ${TARGET_POSTCODE_PREFIX}...`);
  
  const outputFile = path.join(ALIGNED_DATA_DIR, 'land-registry-target-postcode.csv');
  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(LAND_REGISTRY_FILE)
      ...
      .on('data', (row) => {
        ...
        const postcode = (row.postcode || '').trim();
        if (!postcode.startsWith(TARGET_POSTCODE_PREFIX)) return;
...
async function filterEPCByPostcode() {
  console.log(`📖 Filtering EPC data by postcode prefix: ${TARGET_POSTCODE_PREFIX}...`);
  
  const outputFile = path.join(ALIGNED_DATA_DIR, 'epc-target-postcode.csv');
  const csvWriter = createObjectCsvWriter({
    path: outputFile,
    header: [
...
async function buildUnifiedSample() {
  console.log('📖 Building unified sample with postcode focus...');
  
  const outputFile = path.join(ALIGNED_DATA_DIR, 'unified-sample-final.csv');
  const landRegistryFile = path.join(ALIGNED_DATA_DIR, 'land-registry-target-postcode.csv');
  const epcFile = path.join(ALIGNED_DATA_DIR, 'epc-target-postcode.csv');
...
async function main() {
  try {
    ...
    console.log(`🎯 Focusing on postcode prefix: ${TARGET_POSTCODE_PREFIX}`);
    
    // Filter and clean all datasets by postcode focus
    await filterLandRegistryByPostcode();
    await filterEPCByPostcode();
    await copyHPIData();
    
    // Build unified sample
    await buildUnifiedSample();
    
    console.log('\n🎉 Final unified dataset complete!');
    console.log(`📁 Data saved to: ${ALIGNED_DATA_DIR}`);
    console.log('📊 Expected high EPC matching rates with postcode focus!');
