...
const ALIGNED_DATA_DIR = 'data/aligned-2006-geographic';
...
// Focus on areas with good EPC coverage based on our analysis
...
console.log('🚀 Building geographic-focused aligned dataset...');
...
async function filterLandRegistryByCounty() {
  console.log('📖 Filtering Land Registry data by target counties...');
  
  const outputFile = path.join(ALIGNED_DATA_DIR, 'land-registry-target-counties.csv');
  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(LAND_REGISTRY_FILE)
      ...
      .on('data', (row) => {
        ...
        if (!TARGET_COUNTIES.includes(county)) return;
...
async function filterEPCByCounty() {
  console.log('📖 Filtering EPC data by target counties...');
  
  const outputFile = path.join(ALIGNED_DATA_DIR, 'epc-target-counties.csv');
  const csvWriter = createObjectCsvWriter({
    path: outputFile,
    header: [
...
async function buildUnifiedSample() {
  console.log('📖 Building unified sample with geographic focus...');
  
  const outputFile = path.join(ALIGNED_DATA_DIR, 'unified-sample-geographic.csv');
  const landRegistryFile = path.join(ALIGNED_DATA_DIR, 'land-registry-target-counties.csv');
  const epcFile = path.join(ALIGNED_DATA_DIR, 'epc-target-counties.csv');
...
async function main() {
  try {
    ...
    console.log(`🎯 Focusing on counties: ${TARGET_COUNTIES.join(', ')}`);
    
    // Filter and clean all datasets by geographic focus
    await filterLandRegistryByCounty();
    await filterEPCByCounty();
    await copyHPIData();
    
    // Build unified sample
    await buildUnifiedSample();
    
    console.log('\n🎉 Geographic-focused aligned dataset complete!');
    console.log(`📁 Data saved to: ${ALIGNED_DATA_DIR}`);
    console.log('📊 Expected much higher EPC matching rates with geographic focus!');
