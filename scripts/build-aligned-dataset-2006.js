...
const ALIGNED_DATA_DIR = 'data/aligned-2006';
...
const START_YEAR = 2006; // Align all data from 2006 onwards

console.log('🚀 Building aligned dataset starting from 2006...');
...
async function filterLandRegistryData() {
  console.log('📖 Filtering Land Registry data from 2006 onwards...');
  
  const outputFile = path.join(ALIGNED_DATA_DIR, 'land-registry-2006.csv');
    ...
    fs.createReadStream(LAND_REGISTRY_FILE)
      ...
      .on('data', (row) => {
        ...
        if (totalProcessed % 100000 === 0) {
          console.log(`  Processed ${totalProcessed.toLocaleString()} records, filtered ${filteredCount.toLocaleString()}...`);
        }
      ...
      .on('end', () => {
        console.log(`✅ Land Registry filtering complete: ${filteredCount.toLocaleString()} records (${totalProcessed.toLocaleString()} total)`);
...
async function filterEPCData() {
  console.log('📖 Filtering EPC data from 2006 onwards...');
  
  const outputFile = path.join(ALIGNED_DATA_DIR, 'epc-2006.csv');
  const csvWriter = createObjectCsvWriter({
    path: outputFile,
    header: [
  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(EPC_FILE)
      ...
      .on('data', (row) => {
        ...
        if (inspectionDate && inspectionDate.match(/^\d{4}-\d{2}-\d{2}$/)) {
      ...
      .on('end', () => {
        console.log(`✅ EPC filtering complete: ${filteredCount.toLocaleString()} records (${totalProcessed.toLocaleString()} total)`);
        resolve(filteredCount);
      })
      .on('error', reject);
  });
...
async function buildUnifiedSample() {
  console.log('📖 Building unified sample with aligned data...');
  
  const outputFile = path.join(ALIGNED_DATA_DIR, 'unified-sample-2006.csv');
  const landRegistryFile = path.join(ALIGNED_DATA_DIR, 'land-registry-2006.csv');
  const epcFile = path.join(ALIGNED_DATA_DIR, 'epc-2006.csv');
...
async function main() {
  try {
    ...
    // Filter and align all datasets
    await filterLandRegistryData();
    await filterEPCData();
    await copyHPIData();
    ...
    console.log('\n🎉 Aligned dataset complete!');
    console.log(`📁 Data saved to: ${ALIGNED_DATA_DIR}`);
    console.log('📊 Expected much higher EPC matching rates with 2006+ data!');
