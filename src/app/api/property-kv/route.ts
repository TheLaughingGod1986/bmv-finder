        ...
        if (paginatedIds.length > 0) {
            ...
            results?.forEach((result) => {
                if (result && typeof result === 'object') {
                    ...
                    const property = { ...result, price: Number((result as { price: string }).price) } as SoldPrice;
...
export async function POST(request: NextRequest) {
    // Restore: Only use mock data in development
    if (process.env.NODE_ENV === 'development') {
        console.log('[DEV MODE] Returning mock property data (POST).');
        await new Promise(resolve => setTimeout(resolve, 500)); // Simulate network delay
        return NextResponse.json({
            data: mockSoldPrices,
            message: 'Mock data for development (POST)',
        });
    }
    try {
        const { postcode, limit = 100, offset = 0 } = await request.json();
        let propertyIds: string[] = [];

        console.log(`[KV API] POST Request received. Postcode param: "${postcode}"`);
        ...
        const total = propertyIds.length;
        console.log(`[KV API] Found a total of ${total} properties to return (POST).`);
        ...
        return NextResponse.json(
            { message: 'Error processing POST request', error: (error as Error).message }, 
            { status: 500 }
        );
    }
} 