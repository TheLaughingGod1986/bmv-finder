...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (propertyId) {
      ...
      const property = await queryRow(`SELECT * FROM prices WHERE id = ?`, [propertyId]);
      if (!property) {
    ...
    if (postcode) {
      ...
      const growthPromises = soldPrices.map(async (sp) => {
        ...
        if (!Array.isArray(rows) || rows.length < 2) return { ...sp, growthPct: null };
        const first = rows[0]?.price;
        const last = rows[rows.length - 1]?.price;
        if (typeof first !== 'number' || typeof last !== 'number') return { ...sp, growthPct: null };
