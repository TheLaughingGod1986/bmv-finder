...
function updateUserInterface() {
  ...
  if (!userData.isAuthenticated) {
    ...
    if (limit === -1) {
      ...
      captureLimitDisplay.textContent = '∞';
      progressFill.style.width = '0%';
    } else {
      captureLimit.textContent = `${currentCount} / ${limit}`;
      captureLimitDisplay.textContent = `${currentCount}/${limit}`;
...
async function loadCapturedProperties() {
  try {
    ...
    if (userProperties.length > 0) {
      ...
      // Use shorter format to fit better
    ...
    // Update the UI after loading properties to ensure consistency
    updateUserInterface();
